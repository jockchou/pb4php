...
class Entry_Assign extends PBMessage
{
    ...
    {
        $arr = array();
        for ($i = 0; $i < $this->_get_arr_size(1); ++$i)
        {
            $arr[] = $this->_get_arr_value(1, $i)->get_value();
        }
        return $arr;
    }
    function add_nottake($value)
    {
        $v = new PBString();
        $v->set_value($value);
        $this->values[1][] = $v;
    }
    function set_nottake($arr_value)
    {
        $this->values[1] = array();
    ...
    function add_take($value)
    {
        $v = new PBString();
        $v->set_value($value);
        $this->values[2][] = $v;
    }
    function set_take($arr_value)
    {
        $this->values[2] = array();
        foreach ($arr_value as $value)
        {
